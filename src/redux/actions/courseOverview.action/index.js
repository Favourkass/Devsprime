...
import {GET_ALL_COURSES, GET_INSTRUCTOR,GET_ALL_COURSE_DETAIL, ADD_TO_CART} from "../types";
 ...
 export const getInstructor = (instructor_id, token) => async(dispatch) => {
    ...
    try{
        const data = await request.get(`/instructors/courses/${instructor_id}/`, headers(token))
 ...
 export const addToCart = (token) => async(dispatch) => {
     
    try{
        const data = await request.get(`/cart/`, headers(token))
